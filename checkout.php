...
if(isset($_POST['place_order'])){
	...
	if($is_error==''){
		...
		$delivery_on=date('Y-m-d', strtotime($added_on. ' + 5 days'));
		$sql="insert into order_master(user_id,name,email,mobile,address,zipcode,total_price,order_status,payment_status,added_on,delivered_on,coupon_code,final_price,payment_type) values('".$_SESSION['FOOD_USER_ID']."','$checkout_name','$checkout_email','$checkout_mobile','$checkout_address','$checkout_zip','$totalPrice','1','pending','$added_on','$delivery_on','$coupon_code','$final_price','$payment_type')";
